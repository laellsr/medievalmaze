...
class Player(pygame.sprite.Sprite):
	def __init__(self, x, y, width, height):
		self.images = {
      		"UP":    [pygame.image.load(img) for img in glob.glob(os.path.join(PATH_TO_ASSETS, f"warrior1/{'cu_'}*.png"))],
        	"DOWN":  [pygame.image.load(img) for img in glob.glob(os.path.join(PATH_TO_ASSETS, f"warrior1/{'cd_'}*.png"))],
        	"LEFT":  [pygame.image.load(img) for img in glob.glob(os.path.join(PATH_TO_ASSETS, f"warrior1/{'cl_'}*.png"))],
         	"RIGHT": [pygame.image.load(img) for img in glob.glob(os.path.join(PATH_TO_ASSETS, f"warrior1/{'cr_'}*.png"))]
        }
		self.direction = 'DOWN'
		self.index = 0
